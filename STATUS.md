...
publication boundary (structural rules + keyed denylist, pre-commit and CI), and
the working method (verification engine, git hooks, process skills).

Next block: the stage contracts (`pipeline/10_normalize`, `20_match`,
`30_tailor`, `60_letter`), the job/match/letter schemas and `cvac stage
list|show|pack`, then the example user rendered end to end.
...
1. Stage contracts + schemas + `cvac stage` (with tests).
2. Example user `robin` through the framework's own stages; CI builds the PDFs.
3. Domain skills and `cvac skills install`.
4. README that earns the positioning; ARCHITECTURE cut to what exists.
...
| `0bb5a7e` | 2026-09-22 | chore: repository bootstrap with licence, packaging and the publication boundary test |
<!-- /AUTO:COMMITS -->
